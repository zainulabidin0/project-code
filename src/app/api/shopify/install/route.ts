...
export async function GET(req: NextRequest) {
  ...
  if (!shopRaw || !projectId) {
    return jsonError("INVALID_INPUT", "shop and projectId are required", 400);
  }
  ...
  const state = Buffer.from(JSON.stringify({ projectId, ts: Date.now() }), "utf8").toString("base64url");
  try {
